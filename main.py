...
def home():
    ...
    if request.method == 'POST':  # При отправке формы
        ...
        try:
            ...
            return render_template('flight_table.html',
...
@app.route(
    '/download/<string:satellite_name>/<string:start_time>/<string:end_time>/<float:latitude>/<float:longitude>/<float:altitude>')
def download(satellite_name, start_time, end_time, latitude: float, longitude: float, altitude: float):
    ...
    return send_file(trajectory_satellite_text,
                     mimetype='text/plain',
                     as_attachment=True,
                     download_name=f"trajectory_{satellite_name}.txt")


def processing_values(form: ParamsForm):
    """Проверка введённых значений"""

    coordinates = [form.latitude.data, form.longitude.data, form.altitude.data]
    horizon_and_culmination = [form.horizon.data, form.minimum_culmination.data]
    schedule_length = [datetime.combine(form.start_data.data, form.start_time.data), form.flight_length.data]

    # Получаем данные полётов
    flight_data = flight_calculation.CalculationsSatellite(coordinates, horizon_and_culmination, schedule_length)
